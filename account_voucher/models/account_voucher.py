...
class AccountVoucher(models.Model):
    ...
    def _check_paid(self):
        for rec in self:
            rec.paid = any([((line.account_id.internal_type, 'in', ('receivable', 'payable')) and line.reconciled) for line in rec.move_id.line_ids])
...
class AccountVoucherLine(models.Model):
    ...
    def _compute_subtotal(self):
        for rec in self:
            ...
            if rec.tax_ids:
                taxes = rec.tax_ids.compute_all(rec.price_unit, rec.voucher_id.currency_id, rec.quantity, product=rec.product_id, partner=rec.voucher_id.partner_id)
